...
export const Voucher = (propos) => {
    ...
    const getAllVouchers = async () => {
        const _unexVouchers = await getUnexecutedVouchers(provider.getSigner(), propos.dappAddress, config[connectedChain.id].graphqlAPIURL);
    ...
    const _executeVoucher = async (voucher) => {
        if (voucher.proof) {

            const newVoucherToExecute = { ...voucher };
            try {
                const signer = await provider.getSigner();
                const tx = await executeVoucher(signer, propos.dappAddress, voucher.input.index, voucher.index, config[connectedChain.id].graphqlAPIURL);
    ...
    useEffect(() => {
        const setvoucher = async (voucher) => {
            ...
            voucher.executed = !(unexVouchers.some(_v => { return _v.index == voucher.index && _v.input.index == voucher.input.index }));
